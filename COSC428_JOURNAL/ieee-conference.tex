...
\hypersetup{
            pdftitle={3D Reconstruction from UAV Images for Plant Phenotyping through incremental SfM},
...
\title{3D Reconstruction from UAV Images for Plant Phenotyping through
incremental SfM}
...
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[breakable, sharp corners, frame hidden, interior hidden, boxrule=0pt, enhanced, borderline west={3pt}{0pt}{shadecolor}]}{\end{tcolorbox}}\fi
...
unstructured images, without the use of references e.g.~GNSS. With
increasing dataset size a higher resolution can be achieved, SfM are
usually able to provide mm-accuracy \autocite{paulus2019}.
...
consists of a drone equipped with a high resolution camera and a
RTK-GNSS localisation system for imaging the plants; second, the
computer vision system, which builds a 3D point cloud of the ground and
its plants.
...
further drone specs, refer to \cref{table_specs}.
...
out a 3D pointcloud. For comparison a 3D reconstruction with the
commercially available PIX4Dmapper is done on the downsampled and
original dataset. Additionally, a densification of the pointcloud
through MVS is done in PIX4Dmapper on the high resolution data set.

If we compare the number of points in each pointcloud in
\cref{tab-tool_performance}, it can be observed, that simple incremental
SfM produces the least amount of points, where as PIX4Dmapper works out
...
this the pointcloud is filtered for outliers, and a plane is fitted to
...
each found peak, to visualise the separate plants in the pointcloud. It
is clearly visual, that this algorithm can not distinguish between
plants, ground or different objects, as no segmentation is performed.
The algorithm is also incapable to adapt to different plant sizes, as
the bounding boxes are fixed sized. Furthermore, fitting the ground to
...
images and finally applying multi-view-stereo for point cloud
...
fitted and plants in respect to the plane searched for. A fixed bounding
box is created for each plant. Because of a missing segmentation, the
algorithm can not distinguish between plants and surrounding points.
Additionally, the fixed-size bounding boxes fail to adapt to plant
dimensions, further inducing errors.
...
points of improvements, to enhance the 3D reconstruction and also the
plant phenotyping itself. The comparison in Section~\ref{sec-results}
showed, that the commercial solution computes denser point clouds than
the open source implementation of the incremental SfM. To improve this,
...
thus a tradeoff between the accuracy and computational effort should be
analysed \autocite{paulus2019,li2022}.

Another way to improve point clouds while at the same time keeping the
required computing power low, is to incorporate a RTK-GNSS signal, which
provides spatial information about a taken image. Taking the location
data into account, a stereo matching and reconstruction can be done
through calculating the baseline of two images from the location data,
after each acquired image. With this approach a realtime reconstruction
on the fly can be achieved \autocite{hasheminasab2020,matsuura2023}. To
work out a more accurate location of each image, and thus a higher
accuracy baseline, ground control points (GPC) either as physical
markers in the field or through feature matching should be investigated.
Through the alignment of each new image, the accuracy of the position
and thereby of the baseline, and finally the stereo matching, can be
improved \autocite{feng2023}.

Recently 3D reconstruction through deep learning approaches like NeRF
\autocite{gao}, which is already used in the agricultural domain in the
form of Zero NeRF \autocite{peat2022}, or gaussian splatting
\autocite{kerbl} have gained considerable attention for its ability to
capture detailed spatial information in submilimeter accuracy from 2D
images.
...
time, a time variable can be induced and thus a growth model is
retrieved \autocite{paulus2019,li2022}. Fruthermore, incorporation of
multispectra cameras can reveal data about the soil, to get a closed
model of plants, their surroundings and their interaction
\autocite{günder2022}.
